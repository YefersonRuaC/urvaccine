<div class="md:flex">
    ...
    --}}
    @if ($campana->vacuna->tipo_user === 'persona')
        <!-- Left Side -->
        <div class="w-full md:w-4/12 md:mx-2 p-3 shadow-md rounded-md">
            ...
            <h1 class="text-green-900 font-bold text-2xl mt-2 mb-1 text-center md:text-left">{{ $campana->titulo }}</h1>
                ...
                <div class="grid md:grid-cols-2 text-md">
                    <div class="md:col-span-1">
                        <div class="grid grid-cols-2">
                            <p class="px-4 py-2 font-semibold text-green-900">Nombre vacuna:</p>
                            <p class="px-2 py-2">{{ $campana->vacuna->nombre }}</p>
                        </div>
                        <div class="grid grid-cols-2">
                            <p class="px-4 py-2 font-semibold text-green-900">Genero:</p>
                            <p class="px-2 py-2 uppercase">{{ $campana->vacuna->genero }}</p>
                        </div>
                        <div class="grid grid-cols-2">
                            <p class="px-4 py-2 font-semibold text-green-900">Edad desde:</p>
                            <p class="px-2 py-2">{{ $campana->vacuna->edad_desde }}</p>
                        </div>
                        <div class="grid grid-cols-2">
                            <p class="px-4 py-2 font-semibold text-green-900">Edad hasta:</p>
                            <p class="px-2 py-2">{{ $campana->vacuna->edad_hasta }}</p>
                        </div>
                        <div class="grid grid-cols-2">
                            <p class="px-4 py-2 font-semibold text-green-900">Hora desde:</p>
                            <p class="px-2 py-2">{{ $hora_desde->format('h:i') }} a.m</p>
                        </div>
                    </div>
                    <div class="md:col-span-1">
                        <div class="grid grid-cols-2">
                            <p class="px-4 py-2 font-semibold text-green-900">Fecha de la jornada:</p>
                            <p class="px-2 py-2">{{ $campana->fecha->toFormattedDateString() }}</p>
                        </div>
                        <div class="grid grid-cols-2">
                            <p class="px-4 py-2 font-semibold text-green-900">Departamento:</p>
                            <p class="px-2 py-2">{{ $campana->departamento }}</p>
                        </div>
                        <div class="grid grid-cols-2">
                            <p class="px-4 py-2 font-semibold text-green-900">Ciudad/Municipio:</p>
                            <p class="px-2 py-2">{{ $campana->municipio }}</p>
                        </div>
                        <div class="grid grid-cols-2">
                            <p class="px-4 py-2 font-semibold text-green-900">Direccion:</p>
                            <p class="px-2 py-2">{{ $campana->direccion }}</p>
                        </div>
                        <div class="grid grid-cols-2">
                            <p class="px-4 py-2 font-semibold text-green-900">Hora hasta:</p>
            ...
            <div class="p-3 shadow-md rounded-md mt-2">
                <div class="grid grid-cols-1 md:grid-cols-4">
                    <div class="list-inside space-y-2 col-span-1 md:col-span-2">
                        <div>
                            <h3 class="text-green-600">Tipo de usuario</h3>
                            <p class="uppercase">{{ $campana->vacuna->tipo_user }}</p>
                        </div>
                        <div>
                            <h3 class="text-green-600">Tipo vacuna</h3>
                            <p class="uppercase">{{ $campana->vacuna->tipo }}</p>
                        </div>
                        <div>
                            <h3 class="text-green-600">Precio</h3>
                            <p class="">${{ number_format($campana->vacuna->precio, 0, '.', ',') }}</p>
                        </div>
                        <div>
                            <div class="text-green-600">Metodo de propagacion</div>
                            <div class="">{{ $campana->vacuna->propagacion }}</div>
                        </div>
                        <div>
                            <h3 class="text-green-600">Sintomas</h3>
                            <p class="">{{ $campana->vacuna->sintomas }}</p>
                        </div>
                    </div>
                    <div class="col-span-1 md:col-span-2 rounded-md border-2 border-gray-50 hover:border-green-300 hover:border-2 transition-colors duration-300">
